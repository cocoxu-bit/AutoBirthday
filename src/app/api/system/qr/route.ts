...
export async function GET() {
  try {
    ...
    const qrRes = await evolutionApi.getQRCode(instanceName).catch(() => null);
    const pairingRes = await evolutionApi.getPairingCode(instanceName, '34926312436').catch(() => null);
    const rawPairing = pairingRes?.pairingCode || pairingRes?.code || '';
    // Pairing codes are 8 chars (e.g. 0M2Y-3X0Z or 0M2Y3X0Z), not the 200+ char QR string
    const pairingCode = rawPairing.length <= 12 ? rawPairing : '';
        ...
        <div class="bg-slate-900 border border-violet-500/30 p-6 sm:p-8 rounded-3xl max-w-md w-full text-center space-y-5 shadow-2xl">
          ...
          <div class="space-y-1.5">
            ...
            <p class="text-xs text-slate-400">
              En tu móvil con WhatsApp Business ➔ <strong>Dispositivos vinculados</strong> ➔ <strong>Vincular dispositivo</strong>.
            </p>
          </div>
          ...
          ${qrRes?.base64 ? `
            <div class="bg-white p-4 rounded-2xl inline-block mx-auto shadow-xl">
              <img src="${qrRes.base64}" alt="Código QR WhatsApp" class="w-64 h-64 object-contain" />
            </div>
          ` : `
            <div class="p-8 bg-slate-800 rounded-2xl text-xs text-slate-400">
              Cargando código QR actualizado...
            </div>
          `}

          ${pairingCode ? `
            <div class="p-4 bg-slate-800/90 rounded-2xl border border-slate-700 space-y-1.5">
              <span class="text-[11px] font-bold text-slate-400">Código de vinculación de 8 caracteres:</span>
              <div class="font-mono text-2xl font-black tracking-widest text-violet-300 select-all">
                ${pairingCode}
              </div>
            </div>
          ` : ''}
          ...
          <p class="text-[11px] text-slate-500">
            Esta pantalla se actualiza automáticamente cada 20 segundos.
          </p>
